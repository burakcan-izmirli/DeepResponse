...
def create_unique_gene_name_df(l1000=False,
    ...
    if l1000:
        ...
        unique_gene_name_list = l1000_gene_list_raw['pr_gene_symbol'].to_list()
    else:
...
def preprocess_gene_expression_data(gene_exp_path='raw/NCI-60_gene_exp_extended_genes_cell_lines_v1_selected.txt',
                                    export_path='raw/NCI-60_gene_exp_cell_lines_preprocessed.csv'):
...
def preprocess_methylation_data(methylation_path='raw/NCI-60_methylation_extended_genes_cell_lines_v1_selected.txt',
                                export_path='raw/NCI-60_methylation_cell_lines_preprocessed.csv'):
    """ Preprocess gene expression data """
    gene_name_df = create_unique_gene_name_df()
    ...
    methylation_df_filtered.fillna(methylation_df_filtered.mean(numeric_only=True), inplace=True)
    methylation_df_filtered.to_csv(export_path, index=False)


def preprocess_cnv_data(cnv_path='raw/NCI-60_cnv_df_extended_genes_cell_lines_v1_selected.txt',
                        export_path='raw/NCI-60_cnv_cell_lines_preprocessed.csv'):
    """ Preprocess gene expression data """
    gene_name_df = create_unique_gene_name_df()
    ...
    cnv_df_filtered.fillna(cnv_df_filtered.mean(numeric_only=True), inplace=True)
    cnv_df_filtered.to_csv(export_path, index=False)


# %%
def create_drug_cell_dataframe(
        drug_cell_path='raw/NCI-60_drug_response_rcellminer_drug_name_nsc_id_cl_name_pgi50_v3_edits_fps_drop.txt',
        drug_smiles_path='raw/NCI-60_drug_name_SMILES_and_ECFP4_v3.txt'):
    """
    ...
    drug_cell_pairs_raw.rename(columns={'pgi50': 'pic50'}, inplace=True)
...
def create_l1000_dataset(gene_exp_path='raw/NCI-60_gene_exp_cell_lines_preprocessed.csv',
                         mutation_path='raw/NCI-60_mutation_cell_lines_preprocessed.csv',
                         methylation_path='raw/NCI-60_methylation_cell_lines_preprocessed.csv',
                         cnv_path='raw/NCI-60_cnv_cell_lines_preprocessed.csv',

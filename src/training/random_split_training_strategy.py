...
class RandomSplitTrainingStrategy(BaseTrainingStrategy):
    ...
    def _train(
        ...
        logging.info(
            "Checkpoint/Early-stop policy (random strategy): monitor=%s mode=%s",
            monitor_metric,
            monitor_mode,
        )
        ...
        if use_ranking:
            logging.info(
                "Ranking regularization (random strategy): ENABLED (weight=%.4f, group_mode=%s, split=%s)",
                ranking_weight,
            ...
            logging.info(
                "Ranking regularization (random strategy): DISABLED (split=%s)",
                split_type,
            )
                ...
                comet_logger.log_metrics(
                    {
                        ...
                        f"{prefix}/train_rank": train_rank,

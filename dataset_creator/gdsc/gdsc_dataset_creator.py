...
class GDSCDatasetCreator(BaseDatasetCreator):
    ...
    def __init__(self, missing_val_threshold: float = 0.75) -> None:
        ...
        self.methylation_cache_path = self.processed_dir / "gdsc_meth_cache.npz"

        self.baseline_gdsc_ids = self.load_reference_drug_ids("gdsc_id")
        self.baseline_cell_lines = self.load_reference_cell_lines()

    def load_cell_line_map(
        self, cosmic_col: str = "cosmic_id", depmap_col: str = "depmap_id"
    ) -> Dict[str, str]:
    ...
    @staticmethod
    def _ensure_log2_expression_scale(
        expr: pd.DataFrame, value_cols: List[str]
    ) -> pd.DataFrame:
        """Apply log2(x+1) only when values appear to be on raw/linear scale."""
        expr = expr.copy()
        expr[value_cols] = expr[value_cols].apply(pd.to_numeric, errors="coerce")
        values = expr[value_cols].to_numpy(dtype=np.float64)
        ...
        if q99 > 50.0:
            expr[value_cols] = np.log2(
                np.clip(
                    expr[value_cols].to_numpy(dtype=np.float64), a_min=0.0, a_max=None
                )
                + 1.0
            )
            logger.info(
                "Applied log2(x+1) transform to GDSC expression (detected linear-scale values)."
            )
        else:
            logger.info(
                "Detected log-like GDSC expression scale; keeping existing values."
            )
        return expr
    ...
    def _load_methylation_sample_map(self) -> Dict[str, str]:
        ...
        sentrix_id_col = self._require_column(
            df.columns,
            {"sentrixid", "sentrix_id"},
            self.methylation_sample_map_path,
            "sentrix_id",
        )
        sentrix_pos_col = self._require_column(
            df.columns,
            {"sentrixposition", "sentrix_position"},
            self.methylation_sample_map_path,
            "sentrix_position",
        )
        cosmic_col = self._require_column(
            df.columns,
            {"cosmicid", "cosmic_id"},
            self.methylation_sample_map_path,
            "cosmic_id",
        ...
        if not mapping:
            raise ValueError(
                "Sample mapping produced no usable Sentrix->COSMIC mappings (cosmic_id all missing)."
            )
        return mapping

    def _load_gene_tss_index(
        self, gene_axis: List[str]
    ...
    def _load_methylation_cache(
        self, gene_axis: List[str], depmap_ids: Set[str]
    ) -> Optional[Dict[str, np.ndarray]]:
        """Load cached methylation lookup when axis and requested cells are compatible."""
        if not self.methylation_cache_path.exists():
            return None
        try:
            cache = np.load(self.methylation_cache_path, allow_pickle=True)
            required = {"gene_axis", "depmap_ids", "matrix"}
            if not required.issubset(cache.files):
                return None

            cached_axis = [str(g) for g in cache["gene_axis"].tolist()]
            if cached_axis != list(gene_axis):
                logger.info("Ignoring methylation cache: gene axis mismatch.")
                return None

            cached_depmap_ids = [str(c) for c in cache["depmap_ids"].tolist()]
            cached_index = {
                depmap_id: idx for idx, depmap_id in enumerate(cached_depmap_ids)
            }
            if not set(depmap_ids).issubset(cached_index.keys()):
                logger.info(
                    "Ignoring methylation cache: requested cells are not fully cached."
                )
                return None

            matrix = np.asarray(cache["matrix"], dtype=np.float32)
            if matrix.shape != (len(cached_depmap_ids), len(gene_axis)):
                logger.info("Ignoring methylation cache: matrix shape mismatch.")
                return None

            lookup = {
                depmap_id: matrix[cached_index[depmap_id]].astype(
                    np.float32, copy=False
                )
                for depmap_id in depmap_ids
            }
            logger.info(
                "Loaded GDSC methylation cache from %s.", self.methylation_cache_path
            )
            return lookup
        except Exception as exc:
            logger.warning(
                "Failed to load methylation cache %s (%s). Recomputing.",
                self.methylation_cache_path,
                exc,
            )
            return None

    def _save_methylation_cache(
        self, gene_axis: List[str], lookup: Dict[str, np.ndarray]
    ) -> None:
        """Persist methylation lookup for fast reuse."""
        if not lookup:
            return
        depmap_ids = sorted(lookup.keys())
        matrix = np.vstack([lookup[depmap_id] for depmap_id in depmap_ids]).astype(
            np.float32, copy=False
        )
        self.methylation_cache_path.parent.mkdir(parents=True, exist_ok=True)
        np.savez(
            self.methylation_cache_path,
            gene_axis=np.asarray(gene_axis, dtype=str),
            depmap_ids=np.asarray(depmap_ids, dtype=str),
            matrix=matrix,
        )
        logger.info("Saved GDSC methylation cache to %s.", self.methylation_cache_path)

    def _compute_methylation_lookup_from_f2(
        self,
        gene_axis: List[str],
        depmap_ids: Set[str],
        cosmic_to_depmap: Dict[str, str],
    ) -> Dict[str, np.ndarray]:
        """Compute gene-level methylation vectors (aligned to gene_axis) from the GDSC F2 matrix."""
        sentrix_to_cosmic = self._load_methylation_sample_map()
        tss_by_chr = self._load_gene_tss_index(gene_axis)

        cosmic_to_depmap = {
            cosmic: depmap
            for cosmic, depmap in cosmic_to_depmap.items()
            if depmap in depmap_ids
        }
        wanted_cosmic = set(cosmic_to_depmap.keys())
        if not wanted_cosmic:
            raise ValueError(
                "No COSMIC IDs available for requested depmap_ids; cannot map methylation columns."
            )
        ...
        with self.methylation_matrix_path.open(
            "r", encoding="utf-8", errors="ignore"
        ) as handle:
        ...
        return {
            depmap_id: out_mat[i].astype(np.float32, copy=False)
            for i, depmap_id in enumerate(selected_depmap_ids)
        }

    def _load_methylation_lookup_from_f2(
        self,
        gene_axis: List[str],
        depmap_ids: Set[str],
        cosmic_to_depmap: Dict[str, str],
    ) -> Dict[str, np.ndarray]:
        """Load or build gene-level methylation vectors from the GDSC F2 matrix."""
        depmap_ids = set(depmap_ids)
        cached_lookup = self._load_methylation_cache(gene_axis, depmap_ids)
        if cached_lookup is not None:
            return cached_lookup

        computed_lookup = self._compute_methylation_lookup_from_f2(
            gene_axis, depmap_ids, cosmic_to_depmap
        )
        self._save_methylation_cache(gene_axis, computed_lookup)
        return computed_lookup

    def _load_cnv_lookup(
        self,
        gene_axis: List[str],
        depmap_ids: Set[str],
        cosmic_to_depmap: Dict[str, str],
        ...
        model_col = self._require_column(
            cnv.columns, {"modelid", "model_id"}, self.cnv_summary_path, "model_id"
        )
        gene_col = self._require_column(
            cnv.columns,
            {"genename", "gene_name", "hgncsymbol", "symbol"},
            self.cnv_summary_path,
            "gene_name",
        )
        value_col = self._require_column(
            cnv.columns,
            {"totalcopynumber", "total_copy_number"},
            self.cnv_summary_path,
            "total_copy_number",
        )
        ...
        lookup = {
            depmap_id: np.full(len(gene_axis), np.nan, dtype=np.float32)
            for depmap_id in depmap_ids
        }
    ...
    def _load_mutation_lookup(
        ...
        lookup = {
            depmap_id: np.zeros(gene_count, dtype=np.float32)
            for depmap_id in depmap_ids
        }
        ...
        if model_col is not None:
            ...
            model_to_cosmic = dict(
                zip(model_list[model_id_col], model_list[cosmic_id_col])
            )
            ...
            raise ValueError(
                f"Mutation file missing model_id/cosmic_id columns: {self.mutation_path}"
            )
        ...
        for chunk in pd.read_csv(
            self.mutation_path, usecols=usecols, chunksize=200000, low_memory=False
        ):
            try:
    ...
    @staticmethod
    def _apply_cnv_log2_ratio_lookup(
        cnv_lookup: Dict[str, np.ndarray], eps: float = 1e-3
    ) -> Dict[str, np.ndarray]:
        ...
        for depmap_id, arr in cnv_lookup.items():
            ...
            with np.errstate(divide="ignore", invalid="ignore"):
                arr_transformed = np.log2(clipped / 2.0).astype(np.float32, copy=False)
            if not np.isfinite(arr_transformed).all():
                replaced_cells += 1
                arr_transformed = np.nan_to_num(
                    arr_transformed, nan=0.0, posinf=0.0, neginf=0.0
                ).astype(np.float32, copy=False)
            cnv_lookup[depmap_id] = arr_transformed
        if replaced_cells:
            logger.warning(
                "Replaced non-finite CNV values after log2 transform for %d cell lines.",
                replaced_cells,
            )
        return cnv_lookup

    @staticmethod
    def _apply_methylation_m_value_lookup(
        methylation_lookup: Dict[str, np.ndarray], eps: float = 1e-3
    ) -> Dict[str, np.ndarray]:
        ...
        for depmap_id, arr in methylation_lookup.items():
            ...
            with np.errstate(divide="ignore", invalid="ignore"):
                arr_transformed = np.log2(arr / (1.0 - arr)).astype(
                    np.float32, copy=False
                )
            if not np.isfinite(arr_transformed).all():
                replaced_cells += 1
            ...
            methylation_lookup[depmap_id] = arr_transformed
        if replaced_cells:
            logger.warning(
                "Replaced non-finite methylation values after M-value transform for %d cell lines.",
                replaced_cells,
            )
        return methylation_lookup

    def _resolve_response_columns(
        ...
        drug_id_col = self._require_column(
            resp.columns,
            {"gdscid", "gdsc_id", "drugid", "drug_id"},
            self.drug_response_path,
            "drug_id",
        )
        ic50_col = self._require_column(
            resp.columns,
            {"ic50um", "ic50_um", "ic50published", "ic50_published", "ic50"},
            self.drug_response_path,
            "ic50",
        )
    ...
    def _normalize_response_table(
        ...
        resp[drug_id_col] = pd.to_numeric(resp[drug_id_col], errors="coerce").astype(
            "Int64"
        )
        ...
        resp["cell_line_name"] = (
            resp[cosmic_col].map(cosmic_to_depmap).fillna(resp[cosmic_col])
        )
        return resp

    def _resolve_drug_columns(self, drugs: pd.DataFrame) -> Tuple[str, str, str]:
        """Resolve required drug columns."""
        drug_smiles_id_col = self._require_column(
            drugs.columns,
            {"gdscid", "gdsc_id", "drugid", "drug_id"},
            self.drug_smiles_path,
            "drug_id",
        )
        drug_name_col = self._require_column(
            drugs.columns,
            {"drugname", "drug_name", "name"},
            self.drug_smiles_path,
            "drug_name",
        )
        smiles_col = self._require_column(
            drugs.columns,
    ...
    def _normalize_drug_table(
        self,
        drugs: pd.DataFrame,
        drug_smiles_id_col: str,
        drug_name_col: str,
        smiles_col: str,
    ) -> pd.DataFrame:
        """Normalize drug table values."""
        drugs[drug_smiles_id_col] = pd.to_numeric(
            drugs[drug_smiles_id_col], errors="coerce"
        ).astype("Int64")
        drugs = drugs.dropna(subset=[drug_smiles_id_col, drug_name_col]).copy()
        drugs["drug_name"] = (
            drugs[drug_name_col].astype(str).apply(BaseDatasetCreator.clean_string)
        )
        drugs["smiles"] = drugs[smiles_col].apply(self.normalize_smiles_text)
        return drugs.drop_duplicates(subset=[drug_smiles_id_col])

    def _apply_baseline_filters(
        self, merged: pd.DataFrame, drug_id_col: str
    ) -> pd.DataFrame:
        ...
        if self.baseline_cell_lines:
            merged = merged[
                merged["cell_line_name"].isin(self.baseline_cell_lines)
            ].copy()
        return merged

    def _finalize_response_records(
        self, merged: pd.DataFrame, ic50_col: str
    ) -> pd.DataFrame:
    ...
    def _load_drug_response_records(
        self,
        cosmic_to_depmap: Dict[str, str],
        combined_dict: Optional[Dict[str, str]] = None,
    ) -> pd.DataFrame:
        """Load and filter drug response records with SMILES and pic50."""
        resp = self._load_clean_table(self.drug_response_path)
        cosmic_col, drug_id_col, ic50_col, dataset_col = self._resolve_response_columns(
            resp
        )
        resp = self._normalize_response_table(
            resp, cosmic_col, drug_id_col, ic50_col, cosmic_to_depmap
        )

        drugs = self._load_clean_table(self.drug_smiles_path)
        drug_smiles_id_col, drug_name_col, smiles_col = self._resolve_drug_columns(
            drugs
        )
        drugs = self._normalize_drug_table(
            drugs, drug_smiles_id_col, drug_name_col, smiles_col
        )

        merged = resp.merge(
            drugs[[drug_smiles_id_col, "drug_name", "smiles"]],
            left_on=drug_id_col,
        ...
        if combined_dict:
            merged["drug_name"] = (
                merged["drug_name"]
                .astype(str)
                .map(lambda name: combined_dict.get(name, name))
            )
        merged = self._dedupe_by_dataset_preference(
            merged, ["cell_line_name", drug_id_col], dataset_col
        )
        merged["drug_id"] = pd.to_numeric(merged[drug_id_col], errors="coerce")
        merged = self._apply_pubchem_smiles_fallback(merged)
        merged["smiles"] = merged["smiles"].apply(self.normalize_smiles_text)
        merged["_molecule_identity"] = merged["smiles"].apply(
            self.normalize_smiles_text
        )
        merged["_molecule_identity"] = merged["_molecule_identity"].fillna(
            merged["drug_name"].astype(str)
        )
        merged = self._filter_to_preferred_dataset(
            merged,
            ["cell_line_name", "_molecule_identity"],
            dataset_col,
            preferred_value="GDSC2",
        )
        merged = merged.drop(columns=["_molecule_identity"])
    ...
    def _load_expression_matrix(
        ...
        header_cells = self._read_header_fields(
            self.expression_path, delimiter=expr_sep
        )
        ...
        depmap_to_cosmic = {
            depmap: cosmic for cosmic, depmap in cosmic_to_depmap.items() if depmap
        }
        wanted_cosmic = sorted(
            {
                depmap_to_cosmic.get(x)
                for x in wanted_depmap
                if depmap_to_cosmic.get(x) is not None
            }
        )
        ...
        expr[gene_col] = (
            expr[gene_col].astype(str).apply(BaseDatasetCreator.clean_string)
        )
        ...
        if use_cosmic:
            cosmic_to_depmap_effective = {
                depmap_to_cosmic.get(d): d
                for d in wanted_depmap
                if depmap_to_cosmic.get(d)
            }
            expr = expr.rename(columns=cosmic_to_depmap_effective)
        if expr.columns.duplicated().any():
            expr = expr.groupby(level=0, axis=1).mean()
        expr = self._ensure_log2_expression_scale(expr, list(expr.columns))
        return expr
    ...
    def _build_cell_line_features(
        ...
        methylation_lookup: Dict[str, np.ndarray],
    ) -> pd.DataFrame:
        ...
        all_cells = (
            cells_with_expression.union(cells_with_mutation)
            .union(cells_with_cnv)
            .union(cells_with_meth)
        )
        eligible_cells = all_cells.intersection(depmap_ids)
        if not eligible_cells:
            raise ValueError(
                "No GDSC cells with omics + drug response after alignment."
            )

        cell_features = []
        for depmap_id in sorted(eligible_cells):
            if depmap_id in expr.columns:
                gene_expression = pd.to_numeric(
                    expr[depmap_id], errors="coerce"
            ...
            tensor = np.stack([gene_expression, mut, cnv, meth], axis=0).astype(
                np.float32, copy=False
            )
    ...
    def create_dataset(self) -> pd.DataFrame:
        ...
        if not cosmic_to_depmap:
            raise ValueError(
                "Building GDSC features requires gdsc_cell_lines.csv mapping COSMIC_ID to DepMap IDs "
                f"at {self.cell_line_map_path}."
            )

        records = self._load_drug_response_records(
            cosmic_to_depmap, combined_dict=combined_dict
        )
        records = self._harmonize_reference_smiles(records, reference_smiles_map)
        if combined_dict:
            records = self._apply_canonical_smiles(
                records, canonical_smiles_map, combined_dict, reference_smiles_map
            )
        records["smiles"] = records["smiles"].apply(self.normalize_smiles_text)
        records = records.dropna(subset=["smiles"]).reset_index(drop=True)
        ...
        cnv_lookup = self._load_cnv_lookup(
            gene_axis, depmap_ids_requested, cosmic_to_depmap
        )
        cnv_lookup = self._apply_cnv_log2_ratio_lookup(cnv_lookup)
        mutation_lookup = self._load_mutation_lookup(
            gene_axis, depmap_ids_requested, cosmic_to_depmap
        )
        methylation_lookup = self._load_methylation_lookup_from_f2(
            gene_axis, depmap_ids_requested, cosmic_to_depmap
        )
